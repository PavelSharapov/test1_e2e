...
let LoginedUserTournamentsPage = require('../PageObgects/tournamentsPageLogined-objects');
...
describe('Tournaments page sections.', function () {
    let loginedUserTournamentsPage = new LoginedUserTournamentsPage();
    let headerFooterLogined = new HeaderFooterLoginedUser();
    // let logInHelper = new Login();
    // logInHelper.logIn('p1', '44445678t');   // Login to site


    it('open Tournaments page', function () {
        loginedUserTournamentsPage.getTournamentsPage();
    });
    describe('In progress modal:', function () {
        it('should displayed "In progress" modal', function () {
            expect(loginedUserTournamentsPage.inProgresModal.isDisplayed()).toBe(true);
        });
    });
    ...
    describe('Footer section elements:', function () {
        ...
        it('should displayed "Wirecard" image ',function () {
            ...
            /*loginedUserTournamentsPage.footerWirecardImage.getAttribute('src').then(function (img) {
                console.log(img);
            });*/
        });
        it('should displayed scroll-top buton',function () {
            loginedUserTournamentsPage.jumpToFooter();
            expect(headerFooterLogined.footerScrollTopButton).toBeDisplayed();
            loginedUserTournamentsPage.jumpToHeader();
        });
    });
});
